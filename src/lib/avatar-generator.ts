/**
 * Avatone - Unique Avatar Generator
 * Generates consistent, deterministic avatars based on input strings
 * Uses duotone color palettes with abstract patterns
 */

import { createHash } from 'crypto';
import { DUOTONE_PALETTES, PATTERN_COUNT } from './avatar-constants';

/**
 * Generate consistent numeric hashes from any input string
 * Returns two independent values for better distribution
 */
function hashString(input: string): { hash1: number, hash2: number } {
  const hash = createHash('md5').update(input).digest('hex');
  // Use different parts of the hash for independent values
  const hash1 = parseInt(hash.substring(0, 8), 16);
  const hash2 = parseInt(hash.substring(8, 16), 16);
  return { hash1, hash2 };
}

/**
 * Generate avatar SVG based on input string
 */
export function generateAvatar(input: string): string {
  const { hash1, hash2 } = hashString(input);
  // Use independent hash values for better distribution
  const paletteIndex = hash1 % DUOTONE_PALETTES.length;
  const patternIndex = hash2 % PATTERN_COUNT;
  
  const palette = DUOTONE_PALETTES[paletteIndex];
  const pattern = generatePattern(patternIndex, palette);
  
  // Use a unique ID based on the input hash to avoid conflicts
  const clipId = `avatar-clip-${hash1}-${hash2}`;
...
/**
 * Generate pattern based on index and palette
 */
function generatePattern(patternType: number, palette: string[]): string {
  let pattern = '';
  ...
  switch (patternType) {
      ...
      pattern = `
        ...
        ${Array.from({ length: 5 }, (_, i) => {
          return Array.from({ length: 5 }, (_, j) => {
            if ((i + j) % 2 === 1) {
              // Only draw colored squares, let white background show through for others
    ...
    case 20: // Wave curves (smooth) - USER FAVORITE
      pattern = `
        <rect x="0" y="0" width="100" height="100" fill="${palette[0]}"/>
    ...
    case 21: // Wave interference - USER FAVORITE
      pattern = `
        <rect x="0" y="0" width="100" height="100" fill="${palette[0]}"/>
    ...
    case 22: // Sine wave layers - USER FAVORITE
      pattern = `
        <rect x="0" y="0" width="100" height="100" fill="${palette[0]}"/>
        ${Array.from({ length: 4 }, (_, i) => {
          const y = 20 + i * 15;
    ...
    default:
      // Fallback to simple stripes
      pattern = `
        <rect x="0" y="0" width="100" height="100" fill="${palette[0]}"/>
        <rect x="0" y="0" width="100" height="33" fill="${palette[1]}"/>
        <rect x="0" y="33" width="100" height="34" fill="${palette[2]}"/>
        <rect x="0" y="67" width="100" height="33" fill="${palette[3]}"/>
      `;
  }
